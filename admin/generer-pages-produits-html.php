<?php
/**
 * Générateur de Pages Produits HTML Statiques
 * Génère des pages HTML optimisées SEO à partir du CSV
 */
...
echo "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Génération Pages Produits</title>";
...
echo "<h1>🏭 Génération des Pages Produits HTML</h1>";
echo "<p>Lecture du fichier: <code>$csvFile</code></p><hr>";
...
while (($row = fgetcsv($file)) !== false) {
    ...
    if (file_put_contents($fileName, $html)) {
        echo "<div class='success'>✓ Généré: <strong>{$produit['NOM_PRODUIT']}</strong> → $fileId.html</div>";
        $stats['generated']++;
    } else {
        echo "<div class='error'>✗ Erreur: {$produit['NOM_PRODUIT']}</div>";
        $stats['errors']++;
    }
}
...
echo "⊘ Ignorées: {$stats['skipped']}\n";
echo "━━━━━━━━━━━━━━━━━━━━━━\n";
echo "TOTAL: " . ($stats['generated'] + $stats['errors'] + $stats['skipped']);
echo "</pre>";
echo "<p><a href='/admin/produits.php'>← Retour à la gestion produits</a></p>";
echo "</body></html>";
